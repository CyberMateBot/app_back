...
| План | ID | Цена | Монет / период | ₽ за монету | Ранг |
|------|-----|------|----------------|-------------|------|
| Старт | `free` | 0 ₽ | 15 | — | 0 |
| Базовый | `basic` | 149 ₽/мес | 160 | 0.93 | 1 |
| Про | `pro` | 349 ₽/мес | 400 | 0.87 | 2 |
| Максимум | `max` | 799 ₽/мес | 950 | 0.84 | 3 |
| Бизнес | `ultra` | 1999 ₽/мес | 2600 | 0.77 | 4 |

> Монеты при покупке подписки начисляются один раз за период (или при ручной выдаче админом с флагом `grant_coins`).
>
> **Принцип баланса (обновлён 2026-07-24):** размер начисления монет подобран так, чтобы каждый тариф позволял несколько раз в месяц воспользоваться своей ключевой возможностью (ранее месячного лимита не хватало даже на одно видео). Цена за монету снижается с ростом тарифа, поэтому более дорогая подписка — всегда самый выгодный способ получить монеты, а разовые пакеты стоят чуть дороже подписки за монету. Так как цены генераций заложены с наценкой ~3.5× к стоимости провайдера, одна потраченная монета обходится сервису ≈ 0.29 ₽, что даёт валовую маржу ~63–69% даже при полном расходе лимита.
...
| ID | Монеты | Цена | ₽ за монету |
|----|--------|------|-------------|
| pack-100 | 100 | 129 ₽ | 1.29 |
| pack-300 | 300 | 349 ₽ | 1.16 |
| pack-1000 | 1000 | 1049 ₽ | 1.05 |
| pack-2500 | 2500 | 2399 ₽ | 0.96 |

> Пакеты — это разовое пополнение поверх подписки; за монету они стоят чуть дороже подписки соответствующего объёма, но крупные пакеты приближаются к базовому курсу 1 ₽. Монеты из пакета расходуются только на модели, разблокированные текущим тарифом (пакет не открывает доступ к моделям сам по себе).
